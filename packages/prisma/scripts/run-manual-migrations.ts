...
async function runSingleMigration(migrationFile: string, force: boolean = false) {
  ...
  try {
    ...
    if (isIdempotentError) {
      ...
      if (force) {
        console.log(`⚠️  Force mode: Continuing despite error...\n`);
        return false;
      }
      throw error;
    }
...
async function runManualMigrations(specificMigration?: string, force: boolean = false) {
  ...
  for (const migrationFile of MANUAL_MIGRATIONS) {
    try {
      ...
      if (force) {
        console.error(`⚠️  Migration ${migrationFile} failed, but continuing (force mode)...\n`);
        continue;
      }
      throw error;
    }
...
async function main() {
  ...
  try {
    ...
    if (!force) {
      console.error('\n💡 Tip: Use --force flag to continue despite errors');
      console.error('💡 Tip: Run specific migration: tsx run-manual-migrations.ts 004_partial_index');
    }
    process.exit(1);
  }
}
